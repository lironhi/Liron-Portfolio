...
workflow.add_node("detector", detector_agent)      # 🔍 Threat Detection
workflow.add_node("investigator", investigator_agent)  # 🔬 Event Correlation
workflow.add_node("reporter", reporter_agent)      # 📋 Response & Reporting
...
## 🔍 Detectable Incident Categories
...
```
AI-Security-Log-Analyzer-Agent/
├── app/                    # FastAPI server and REST API
│   ├── main.py            # API routes and endpoints
│   ├── models.py          # Pydantic data models
│   └── config.py          # Configuration management
├── agents/                 # LangGraph agent implementations
│   ├── detector.py        # Detector Agent (threat identification)
│   ├── investigator.py    # Investigator Agent (event correlation)
│   └── reporter.py        # Reporter Agent (response & reporting)
├── mcp_tools/             # Model Context Protocol tools
│   ├── log_parser.py      # Log format parsing
│   ├── vector_db.py       # FAISS vector database
│   ├── ip_intel.py        # IP intelligence and geolocation
│   └── actions.py         # Defensive action executors
